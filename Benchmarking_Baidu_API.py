...
emotions_dic = {"surprise":0, "fear":1, "disgust":2, "happy":3, "sad":4, "angry":5, "neutral":6}
emotions_dic_non = {"surprise":0, "fear":1, "disgust":2, "happy":3, "sad":4, "angry":5, "neutral":6, "None":7}
...
for ope_index in range(19, 31):
    operation = operations[24]
    ope_path = operations_path[24]
    print(operation)
    ...
    for filepath in test_data:
        ...
        f = open(filepath,'rb')
        img_info = base64.b64encode(f.read())

        params = {'image':''+ str(img_info,'utf-8') + '','image_type':'BASE64','face_field':'emotion'}
        ...
        # print(json.loads(content)["error_msg"])
        if json.loads(content)["error_msg"] == "SUCCESS":
            content = ast.literal_eval(content)
            label_analysis.append(content["result"]["face_list"][0]["emotion"])
            # print(content["result"]["face_list"][0]["emotion"])
        else:
            label_analysis.append(json.loads(content)["error_msg"])
    ...
    for analysis in label_analysis:
        if analysis != "pic not has face":
            label_test.append(analysis["type"])
        else:
    ...
    # save data
    with open("D:\\phd\\code\\Project1\\report\\Experiment_3\\Baidu_API_accuracy_results\\" + str(operation) + ".txt",
                  "w") as f:
    ...
    file1 = open("D:\\phd\\code\\Project1\\API_results\\Experiment_3_" + ope_path + "\\Baidu_API_label_analysis.txt", "w")
    for item in label_analysis:
        file1.write(str(item))
        file1.write("\n")
    file1.close()

    file2 = open("D:\\phd\\code\\Project1\\API_results\\Experiment_3_" + ope_path + "\\Baidu_API_test_data.txt", "w")
    for index in range(np.array(test_data).shape[0]):
        file2.write(str(test_data[index]) + " " + str(test_labels[index]))
        file2.write("\n")
    file2.close()

    file3 = open("D:\\phd\\code\\Project1\\API_results\\Experiment_3_" + ope_path + "\\Baidu_API_label_test.txt", "w")
    for item in label_test:
        file3.write(item)
        file3.write("\n")
    file3.close()

    file4 = open("D:\\phd\\code\\Project1\\API_results\\Experiment_3_" + ope_path + "\\Baidu_API_err_list.txt", "w")
    ...
    if non != 0:
        file5 = open("D:\\phd\\code\\Project1\\API_results\\Experiment_3_" + ope_path + "\\Baidu_API_non_list.txt", "w")
    ...
    with open("D:\\phd\\code\\Project1\\report\\Experiment_3\\Baidu_API_accuracy_results\\" + str(
    ...
    if non != 0:
        ...
        plt.savefig(
            "D:\\phd\\code\\Project1\\report\\Experiment_3\\Baidu_API_results_pictures\\" + operation + "_barplot.png")
        plt.close()
        ...
        plt.savefig(
            "D:\\phd\\code\\Project1\\report\\Experiment_3\\Baidu_API_results_pictures\\" + operation + "_heatmap.png")
        plt.close()
